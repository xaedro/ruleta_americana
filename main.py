...
async def websocket_users(websocket: WebSocket):
	...
	try:
		while True:
			...
			if msg_type == "ping":
				await websocket.send_text(json.dumps({"type": "pong"}))
				logger.debug(f"Enviado pong a {client_id}")
				continue
				...
				if existing_client_id:
					# Enviar ping para verificar si la conexión existente está activa
					logger.info(f"Nombre duplicado '{username}' encontrado para {existing_client_id}, enviando ping")
					try:
						await manager.active_connections[existing_client_id]["websocket"].send_text(json.dumps({"type": "ping"}))
						async with asyncio.timeout(5):
							message = await manager.active_connections[existing_client_id]["websocket"].receive_text()
							data_received = json.loads(message)
							if data_received.get("type") == "pong":
								logger.info(f"Cliente {existing_client_id} respondió al ping, está activo")
								await websocket.send_text(json.dumps({
									"type": "error",
									"message": f"Ya estás logueado con el nombre '{username}'"
								}))
					...
					for cid, info in manager.active_connections.items():
						if info["username"] == username and cid != client_id:
							logger.info(f"Nombre '{username}' sigue en uso por {cid} tras limpieza")
							await websocket.send_text(json.dumps({
								"type": "error",
								"message": f"Ya estás logueado con el nombre '{username}'"
							}))
...
async def websocket_users(websocket: WebSocket):
	...
	try:
		while True:
			...
			if msg_type == "ping":
				logger.warning(f"Ping no solicitado recibido de {client_id}, ignorando")
				continue
						...
						async with asyncio.timeout(5):
							while True:
								...
								if data_received.get("type") == "pong":
									...
									logger.warning(f"Cliente {existing_client_id} envió mensaje inesperado durante ping: {data_received}, continuando espera de pong")
									continue
					except asyncio.TimeoutError:
						logger.info(f"Cliente {existing_client_id} no respondió al ping, desconectando")
						manager.disconnect(existing_client_id)
